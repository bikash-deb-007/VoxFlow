...
class SettingsWindow:
    def __init__(self, tk_root: tk.Tk, on_save_callback=None):
        self._root = tk_root
        self.on_save = on_save_callback
        self._win: tk.Toplevel | None = None

    def open(self) -> None:
        self._root.after(0, self._build)
    ...
    def _build(self) -> None:
        ...
        def row(label, widget_factory):
            ...
            wgt = widget_factory(frame)
            wgt.pack(side="left", fill="x", expand=True)
            return wgt
        ...
        lang_var = tk.StringVar(value=lang_display.get(cfg.get("language"), "Auto-detect"))
        ...
        row("Trigger key:", lambda f: ttk.Entry(f, textvariable=trigger_var, width=16))
